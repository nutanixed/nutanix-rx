...
#   8. Stop PE Cluster         → stop_pe_cluster.sh
#   9. Shutdown CVMs           → shutdown_cvm.sh
#  10. Shutdown AHV            → shutdown_ahv.sh
#  11. Power Off Hosts        → shutdown_hosts_redfish.sh
#
...
#   WAIT_AFTER_USER_VMS       default 120  — after user VM shutdown
...
WAIT_AFTER_USER_VMS="${WAIT_AFTER_USER_VMS:-120}"
...
require_root() {
  ...
  for f in shutdown_ndk_webhook.sh shutdown_nkp_cluster.sh stop_fs_cluster.sh shutdown_fsnkp.sh shutdown_all_vms.sh \
           stop_pc_cluster.sh shutdown_pcvm_api.sh stop_pe_cluster.sh shutdown_cvm.sh \
...
main() {
  ...
  run_step "7/11 Stop PE Cluster" "stop_pe_cluster.sh"
  wait_phase "after PE cluster stop" "$WAIT_AFTER_PE_CLUSTER"

  run_step "8/11 Shutdown CVMs" "shutdown_cvm.sh"
  wait_phase "after CVM shutdown" "$WAIT_AFTER_CVM"

  run_step "9/11 Shutdown AHV" "shutdown_ahv.sh"
  wait_phase "after AHV shutdown" "$WAIT_AFTER_AHV"

  run_step "10/11 Power Off Hosts" "shutdown_hosts_redfish.sh"
